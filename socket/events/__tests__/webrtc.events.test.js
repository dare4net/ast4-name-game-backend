const setupWebRTCEvents = require('../webrtc.events');
...
describe('WebRTC Events', () => {
  ...
  it('should handle WebRTC data messages and update game state', () => {
    ...
    expect(socket.emit).toHaveBeenCalledWith('game:letter_selected', {
    ...
    expect(socket.to).toHaveBeenCalledWith('test-game-id');
    expect(socket.emit).toHaveBeenCalledWith('game:vote_submitted', {
      playerId: 'test-socket-id',
      word: 'testword',
      vote: 'yes'
    });
